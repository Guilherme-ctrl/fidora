-- Atomic, idempotent imports for the Finora JSON contract generated by ChatGPT.

create or replace function private.invoice_amount_impact(p_amount numeric, p_movement text)
returns numeric language sql immutable as $$
  select case
    when p_movement in ('refund', 'credit') then -abs(p_amount)
    when p_movement = 'transfer' then 0
    else abs(p_amount)
  end;
$$;

create or replace function private.parse_import_timestamp(p_value text)
returns timestamptz language sql stable as $$
  select case
    when p_value ~ '^\d{4}-\d{2}-\d{2}$'
      then (p_value::date::timestamp + interval '12 hours') at time zone 'America/Sao_Paulo'
  ...
  if tg_op = 'DELETE' then
    ...
    if target_invoice is not null then
      update public.invoices
         set total = coalesce((
           select sum(private.invoice_amount_impact(amount, movement_type))
             from public.transactions
            where invoice_id = target_invoice and status <> 'ignored'
         ), 0)
  ...
  if tg_op = 'UPDATE' and old.invoice_id is distinct from new.invoice_id and old.invoice_id is not null then
    update public.invoices
       set total = coalesce((
         select sum(private.invoice_amount_impact(amount, movement_type))
           from public.transactions
          where invoice_id = old.invoice_id and status <> 'ignored'
       ), 0)
...
begin
  ...
  if p_payload->>'schema_version' <> '1.0' then raise exception 'unsupported_schema'; end if;
  ...
  for item in select value from jsonb_array_elements(p_payload->'transactions') loop
    ...
    if coalesce((item->>'needs_review')::boolean, false) then review_count := review_count + 1; end if;
  ...
  for item in select value from jsonb_array_elements(p_payload->'transactions') loop
    ...
    if target_transaction is not null then
      ...
      insert into public.transactions(
        ...
        item->>'movement_type' <> 'transfer', 'chatgpt', item
