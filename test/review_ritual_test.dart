...
/// Returns the cubit the page is using, so an assertion reads the same queue
/// the widget does.
Future<ReviewQueueCubit> _pump(WidgetTester tester, Size size) async {
  ...
  return tester.element(find.byType(ReviewQueuePage)).read<ReviewQueueCubit>();
}
...
void main() {
  ...
  testWidgets('one decision on screen, not the whole pile', (tester) async {
    await withGoldenClock(() async {
      final queue = await _pump(tester, const Size(390, 844));
      final pending = queue.state.dataOrNull ?? const [];
  ...
  testWidgets('settling a group advances the count by its size', (
    ...
    await withGoldenClock(() async {
      final queue = await _pump(tester, const Size(390, 844));
      final before = (queue.state.dataOrNull ?? const []).length;
      ...
      final after = (queue.state.dataOrNull ?? const []).length;
  ...
  testWidgets('Enter settles the group the keyboard is on', (tester) async {
    await withGoldenClock(() async {
      final queue = await _pump(tester, const Size(1280, 900));
      final before = (queue.state.dataOrNull ?? const []).length;
